...
def get_headers():
  return json.dumps(list(sutdcoin.graph.keys()))
  ...
  if request.method == "POST":
    ...
    for txn in txn_list:
      for val in sutdcoin.graph.values():
        if txn in val['block'].merkle_tree.past_transactions:
          nodes, neighbour, _ = val['block'].merkle_tree.get_min_nodes(txn)
          root = val['block'].merkle_tree.get_root()
          res[txn] = {"nodes": nodes, "neighbour": neighbour, "root": root}
    res = json.dumps(res)
    return res, 200
