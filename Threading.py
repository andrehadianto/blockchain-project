...
class MiningWorker(threading.Thread):
    ...
    def run(self):
        ...
        for _ in range(50):
            print(self.user_pk[0:6], "starts mining")
            miner.mine()
            print(self.user_pk[0:6], "stops mining")
...
class TransactionWorker(threading.Thread):
    ...
    def run(self):
        ...
        for _ in range(0, random_transactions):
            ...
            trans = Transaction(self.users[random_sender], self.users[random_receiver], random_amount, "{} send to {}".format(
                self.users[random_sender][0:6], self.users[random_receiver][0:6]))
